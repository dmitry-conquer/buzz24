...
// // =======================================================================================================================================================================================================================
export function menuInitLocation() {
	const htmlEl = document.querySelector('html');
	const bgEls = document.querySelectorAll('._bg');
	// Создаем новый MutationObserver
	const observer = new MutationObserver((mutationsList, observer) => {
		// Проходимся по каждой мутации, которая произошла
		mutationsList.forEach((mutation) => {
			// Проверяем, является ли мутация изменением класса для элемента с классом "_bg"
			if (mutation.type === 'attributes' && mutation.attributeName === 'class' && mutation.target.classList.contains('active') && mutation.target.classList.contains('_bg')) {
				// Если класс "active" был добавлен, добавляем класс "_bg-body" к элементу HTML
				...
				// Если класс "active" был удален, проверяем, остались ли другие элементы с классом "_bg" с классом "active"
				const activeEls = document.querySelectorAll('.active._bg');
				if (activeEls.length === 0) {
					// Если другие элементы с классом "_bg" не имеют класса "active", удаляем класс "_bg-body" из элемента HTML
	...
	// Определяем условие для срабатывания скрипта
	const mediaQuery = window.matchMedia('(min-width: 768px)');
	// const langButton = document.querySelector('.lang-header__button');

	// Функция, которая будет добавлять или удалять обработчик событий, в зависимости от условия
	const handleMediaQuery = (mediaQuery) => {
		if (mediaQuery.matches) {
			// Если условие соответствует, начинаем наблюдать за каждым элементом с классом "_bg"
			bgEls.forEach((el) => {
				observer.observe(el, { attributes: true });
			});
		} else {
			// Если условие не соответствует, прекращаем наблюдение за каждым элементом с классом "_bg"
		//  bgEls.forEach((el) => {
		//    observer.unobserve(el);
		//  });
			// Убираем класс "_bg-body" из элемента HTML
			htmlEl.classList.remove('_bg-body');
		}
	};

	// Вызываем функцию сразу, чтобы установить обработчик для текущего состояния
	handleMediaQuery(mediaQuery);

	// Добавляем обработчик на изменение состояния условия
  ...
  const headerMain = document.querySelector('.header__wrap');
	if (document.querySelector(".icon-menu")) {
		document.addEventListener("click", function (e) {
			...
			else if (!headerMain.contains(e.target)) {
        menuClose();
      }
		});
	};
	...
	if (document.querySelector(".icon-close")) {
		document.addEventListener("click", function (e) {
			if (bodyLockStatus && e.target.closest('.icon-close')) {
				...
				document.documentElement.classList.remove("menu-open");
			} else if (!categoryNav.contains(e.target)) {
        menuCloseC();
      }
		});
	};
...
export function menuOpenC() {
	bodyLock();
	document.documentElement.classList.add("menu-category");
}
export function menuCloseC() {
	bodyUnlock();
