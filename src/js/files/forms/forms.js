...
export let formValidate = {
	...
	// addError(formRequiredItem) {
	// 	formRequiredItem.classList.add('_form-error');
	// 	let parentElement = formRequiredItem.parentElement;
	// 	parentElement.classList.add('_form-error');
	// 	let inputError = parentElement.querySelector('.form__error');
	// 	if (inputError) {
	// 		inputError.textContent = formRequiredItem.dataset.error;
	// 	} else if (formRequiredItem.dataset.error) {
	// 		parentElement.insertAdjacentHTML('beforebegin', `<div class="form__error">${formRequiredItem.dataset.error}</div>`);
	// 	}
	// },
...
export function formSubmit() {
	...
	function formSent(form, responseResult = ``) {
		...
		// 		const popup = form.dataset.popupMessage;
		// 		popup ?
		// 		console.log('dgdff') : console.log('fsdfsdfsfs');
		// 	}
		// }, 0);
		// Очищуємо форму
		formValidate.formClean(form);
